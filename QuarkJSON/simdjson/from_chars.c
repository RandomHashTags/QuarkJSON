...
#define MAX_DIGITS (uint32_t) 768
#define DECIMAL_POINT_RANGE (int32_t) 2047

#define MANTISSA_EXPLICIT_BITS 52
#define MINIMUM_EXPONENT -1023
#define INFINITE_POWER 0x7FF

#define SIGN_INDEX 63

struct adjusted_mantissa {
  uint64_t mantissa;
  int power2;
  //adjusted_mantissa() : mantissa(0), power2(0) {} // TODO: todo what???
};
...
_Bool is_integer(char c) { return (c >= '0' && c <= '9'); }

// This should always succeed since it follows a call to parse_number.
struct decimal parse_decimal(const char *p) {
  struct decimal answer;
  answer.num_digits = 0;
  ...
  while (is_integer(*p)) {
    if (answer.num_digits < MAX_DIGITS) {
        uint8_t value = *p - '0';
        answer.digits[answer.num_digits] = value;
    }
    answer.num_digits++;
    ++p;
  }
  if (*p == '.') {
    ...
    int32_t value = first_after_period - p;
    answer.decimal_point = value;
  }
  if(answer.num_digits > 0) {
    ...
    int32_t digits = answer.num_digits;
    answer.decimal_point += digits;
    uint32_t zeros = trailing_zeros;
    answer.num_digits -= zeros;
  }
  if(answer.num_digits > MAX_DIGITS ) {
...
struct decimal parse_decimal_end(const char *p, const char *end) {
  struct decimal answer;
  answer.num_digits = 0;
  answer.decimal_point = 0;
  answer.truncated = 0;
  ...
  if ((p != end) && (*p == '.')) {
    ...
    int32_t new_decimal_point = first_after_period - p;
    answer.decimal_point = new_decimal_point;
  }
  if(answer.num_digits > 0) {
    const char *preverse = p - 1;
    int32_t trailing_zeros = 0;
...
inline void trim(struct decimal *h) {
  while ((h->num_digits > 0) && (h->digits[h->num_digits - 1] == 0)) {
    h->num_digits--;
  }
}

uint32_t number_of_digits_decimal_left_shift(struct decimal *h, uint32_t shift) {
  shift &= 63;
...
uint64_t round(struct decimal *h) {
...
// computes h * 2^-shift
void decimal_left_shift(struct decimal *h, uint32_t shift) {
  if (h->num_digits == 0) {
    return;
  }
  uint32_t num_new_digits = number_of_digits_decimal_left_shift(h, shift);
  int32_t read_index = h->num_digits - 1;
  uint32_t write_index = h->num_digits - 1 + num_new_digits;
  uint64_t n = 0;

  while (read_index >= 0) {
    uint64_t digits64 = h->digits[read_index];
    n += digits64 << shift;
    uint64_t quotient = n / 10;
    uint64_t remainder = n - (10 * quotient);
    if (write_index < MAX_DIGITS) {
        uint8_t target_remainder = remainder;
        h->digits[write_index] = remainder;
    } else if (remainder > 0) {
        h->truncated = 1;
    }
    n = quotient;
  ...
  while (n > 0) {
    ...
    if (write_index < MAX_DIGITS) {
        uint8_t target_remainder = remainder;
        h->digits[write_index] = target_remainder;
    } else if (remainder > 0) {
        h->truncated = 1;
    }
    n = quotient;
  ...
  int32_t new_digits = num_new_digits;
  h->decimal_point += new_digits;
  trim(h);
}

// computes h * 2^shift
void decimal_right_shift(struct decimal *h, uint32_t shift) {
...
struct adjusted_mantissa compute_float(struct decimal *d) {
  struct adjusted_mantissa answer;
  if (d->num_digits == 0) {
    // should be zero
    answer.power2 = 0;
    answer.mantissa = 0;
  ...
  while ((MINIMUM_EXPONENT + 1) > exp2) {
    uint32_t n = (MINIMUM_EXPONENT + 1) - exp2;
    if (n > max_shift) {
      n = max_shift;
    }
  ...
  // to shift back.
  uint64_t one = 1;
  if (mantissa >= (one << mantissa_size_in_bits)) {
  ...
  if (mantissa < (one << MANTISSA_EXPLICIT_BITS)) {
    answer.power2--;
  }
  answer.mantissa =
      mantissa & ((one << MANTISSA_EXPLICIT_BITS) - 1);
  return answer;
}

struct adjusted_mantissa parse_long_mantissa(const char *first) {
  struct decimal d = parse_decimal(first);
  return compute_float(&d);
}

struct adjusted_mantissa parse_long_mantissa_end(const char *first, const char *end) {
  struct decimal d = parse_decimal_end(first, end);
  return compute_float(&d);
}

double from_chars(const char *first) {
  _Bool negative = first[0] == '-';
  if (negative) {
    first++;
  }
  struct adjusted_mantissa am = parse_long_mantissa(first);
  uint64_t word = am.mantissa;
  word |= (uint64_t) am.power2 << MANTISSA_EXPLICIT_BITS;
  word = negative ? word | ((uint64_t) 1 << SIGN_INDEX) : word;
  double value;
...
double from_chars(const char *first, const char *end) {
  _Bool negative = first[0] == '-';
  if (negative) {
    first++;
  }
  struct adjusted_mantissa am = parse_long_mantissa_end(first, end);
  uint64_t word = am.mantissa;
  word |= ((uint64_t) am.power2) << MANTISSA_EXPLICIT_BITS;
  word = negative ? word | ((uint64_t) 1 << SIGN_INDEX) : word;
  double value;
