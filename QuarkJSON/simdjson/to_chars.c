...
template <typename FloatType> boundaries compute_boundaries(FloatType value) {
  ...
  using bits_type = typename std::conditional<kPrecision == 24, uint32_t,
                                              uint64_t>::type;
  ...
  const _Bool lower_boundary_is_closer = F == 0 && E > 1;
...
satisfies (Definition 3.2 from [1])
     alpha <= e_c + e + q <= gamma.
*/
inline struct cached_power get_cached_power_for_binary_exponent(int e) {
  // Now
  //
  ...
  const struct cached_power cached = kCachedPowers[static_cast<std::size_t>(index)];
...
char *to_chars(char *first, const char *last, double value) {
  ...
  _Bool negative = std::signbit(value);
  if (negative) {
    value = -value;
    *first++ = '-';
  }
