...
void benchmark_stringify(void) {
    ...
    printf("benchmark_stringify; bytes=%lu, length=%lu, least/most GB/s=%f/%f,\n", to_string_length, strlen(to_string), least_bytes_per_nano, most_bytes_per_nano);
}

void benchmark_parsing_from_file(void) {
    struct JSONObject *parsed_json_from_file = alloca(sizeof(struct JSONObject));
    //struct JSONArray parsed_array_from_file;
    ...
    //json_array_parse_from_file("test_array_small.json", &parsed_array_from_file);
    now = clock() - now;
...
void benchmark_simd(void) {
    FILE *file = fopen("test_json_api_weather_gov_fragment.json", "rb");
